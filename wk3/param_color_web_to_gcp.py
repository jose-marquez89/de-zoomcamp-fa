...
def clean(df: pd.DataFrame, color: str) -> pd.DataFrame:
    ...
    if color == "yellow":
        ...
        dt_letter = "l"
        df['trip_type'] = df['trip_type'].fillna(-99).astype("int")
    
    pu_col = f"{dt_letter}pep_pickup_datetime"
    do_col = f"{dt_letter}pep_dropoff_datetime"
    ...
    df[pu_col] = pd.to_datetime(df[pu_col])
    df[do_col] = pd.to_datetime(df[do_col])
...
def etl_web_to_gcs(year: int, month: int, color: str) -> None:
    ...
    cleaned  = clean(df, color)
    path = write_local(cleaned, dataset_file, color)
    write_gcs(path)
